...
CREATE INDEX IF NOT EXISTS items_liked_idx
    ...
    WHERE is_liked = TRUE;
`,
	// 005 — summary title: one-sentence French headline generated by the
	// LLM alongside the bullet list. Kept separate from summary_fr so the
	// dashboard can render title + bullets independently.
	`
ALTER TABLE items
